...
# Added support for cmus by using "playerctl-cmus" script extension of playerctl
# Must suppress all errors 2> /dev/null  

player_status=$(playerctl-cmus status 2> /dev/null | tr '[:upper:]' '[:lower:]')
if [[ $? -eq 0 ]]; then
    metadata="$(playerctl-cmus metadata artist 2> /dev/null ) - $(playerctl-cmus metadata title 2> /dev/null)"
fi
